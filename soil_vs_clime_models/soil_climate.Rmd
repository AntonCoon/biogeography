...
fit <- kmeans(just.parametrs, adm.count)
just.parametrs <- data.frame(just.parametrs, cluster = fit$cluster)

admixture.and.clasters <- data.frame(all_data[, 1:claster.count], cluster = just.parametrs$cluster)
admixture.and.clasters <- admixture.and.clasters[order(admixture.and.clasters$cluster),]

df <- admixture.and.clasters[, 1:claster.count]
...
mdat$Admixture = factor(mdat$Admixture, 
                       levels=names(sort(colSums(df[, 1:claster.count]), decreasing=TRUE)))
...
```{r}
genetic.distance.matrix <- dist(all_data[, 1:14])
...
```{r}
(RoughCorrels <- mantel.rtest(genetic.distance.matrix, geography.distance.matrix, nrepet = 30))
```
